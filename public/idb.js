/**
 * IndexedDB wrapper library for cost management (Vanilla JS version)
...
class CostsDB {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = async () => {
        ...
        const monthCosts = allCosts.filter((cost) => {
          ...
          return (
            costDate.getFullYear() === year && costDate.getMonth() + 1 === month
          );
        });
        ...
        try {
          ...
          for (const cost of monthCosts) {
            const convertedSum = this.convertCurrency(
              cost.sum,
              cost.currency,
              currency,
              exchangeRates
  ...
  getExchangeUrl() {
    return (
      localStorage.getItem("exchangeUrl") ||
...
// Assign to global object for vanilla JS compatibility
