...
const BarChartView = ({ db }) => {
    ...
    return (<Paper elevation={2} sx={{ p: 3 }}>
      ...
      {!loading && (<Box sx={{ height: 450, width: "100%" }}>
          {chartData.length === 0 ||
                chartData.every((item) => item.value === 0) ? (<Alert severity="info">No data found for {year}</Alert>) : (<>
              ...
              <BarChart xAxis={[
                ...
                ]} dataset={chartData} height={350} margin={{ top: 50, right: 30, bottom: 50, left: 80 }}/>
            </>)}
        </Box>)}
    </Paper>);
};
